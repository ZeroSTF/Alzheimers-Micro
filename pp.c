...
void interrupt(){
     if(intcon.inte && intcon.intf)  //interruption test
     {
       ledBuzzer();
       intcon.INTF=0;
     }
...
TRISA.F0=0;
PORTA.F0=0;
TRISB.F4=1; // boutton moteur sens indirect
TRISB.F5=1; // boutton moteur sens direct
TRISC.F0=0;    //MOTEUR
TRISC.F1=0;    //MOTEUR
PORTC.F0=0;    //MOTEUR
PORTC.F1=0;    //MOTEUR
TRISA.F1=1; //Capteur temperature
TRISA.F2=0;
PORTA.F2 =0;
TRISA.F3=0;
PORTA.F3 =0;
...
Temp= ADC_Read(1);
Temp1 = temp * 0.489;
if((Temp1>39)&&(Temp1<42))
        {
             PORTA.F2=1;
        }
    if(Temp1<39&&(Temp1>36)){
             PORTA.F3=1;
        }
